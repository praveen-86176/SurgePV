...
from app.models import Comment, Issue, IssueStatus, Label
...
async def clean_old_resolved_issues(days=90):
    ...
    async with AsyncSessionLocal() as session:
        ...
        for issue in old_issues:
            await session.execute(delete(Comment).where(Comment.issue_id == issue.id))
...
async def remove_unused_labels():
    ...
    async with AsyncSessionLocal() as session:
        ...
        for label in unused_labels:
            await session.execute(delete(Label).where(Label.id == label.id))
