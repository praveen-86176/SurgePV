...
from app.models import Comment, Issue, IssuePriority, IssueStatus
from app.repositories import CommentRepository, IssueRepository, LabelRepository, UserRepository
...
class IssueService:
    ...
    async def get_issue(
        self, issue_id: int, detailed: bool = False
    ...
    async def list_issues(
        ...
        issues, total = await self.issue_repo.get_all(
            skip=skip, limit=page_size, status=status, priority=priority, assignee_id=assignee_id
        )
    ...
    async def add_comment(self, issue_id: int, comment_data: CommentCreate) -> CommentResponse:
        ...
        if not author_exists:
            raise HTTPException(
                status_code=400, detail=f"Author with ID {comment_data.author_id} does not exist"
            )

        # Create comment
        comment = Comment(
            issue_id=issue_id, body=comment_data.body, author_id=comment_data.author_id
        )
    ...
    async def bulk_update_status(self, bulk_data: BulkStatusUpdate) -> BulkStatusUpdateResponse:
        ...
        try:
            updated_count = await self.issue_repo.bulk_update_status(
                bulk_data.issue_ids, bulk_data.status
            )
            await self.db.commit()

            return BulkStatusUpdateResponse(
                updated_count=updated_count, issue_ids=bulk_data.issue_ids, status=bulk_data.status
            )
    ...
    async def import_from_csv(self, file: UploadFile) -> CSVImportResponse:
        ...
        return CSVImportResponse(
            total_rows=total_rows, successful=successful, failed=len(errors), errors=errors
        )
