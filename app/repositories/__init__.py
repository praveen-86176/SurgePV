...
from app.models import Comment, Issue, IssueStatus, Label, User, issue_labels
...
class IssueRepository:
    ...
    async def get_all(
        ...
        """
        # Build base query
        query = select(Issue).options(selectinload(Issue.assignee), selectinload(Issue.labels))
    ...
    async def get_top_assignees(self, limit: int = 10) -> List[tuple[int, int]]:
        ...
        query = (
            ...
            .where(and_(Issue.status == IssueStatus.RESOLVED, Issue.assignee_id.isnot(None)))
    ...
    async def get_resolution_latency(self) -> List[tuple[int, float, int]]:
        ...
        query = (
            select(
                Issue.assignee_id,
                func.avg(func.extract("epoch", Issue.resolved_at - Issue.created_at) / 3600).label(
                    "avg_hours"
                ),
            ...
            .group_by(Issue.assignee_id)
            .order_by(func.avg(func.extract("epoch", Issue.resolved_at - Issue.created_at) / 3600))
        )
